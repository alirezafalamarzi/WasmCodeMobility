...
let src2 = reg_name(**src2);
        ...
        RawInst::XLoad8U32Offset32 { dst,ptr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let ptr = reg_name(**ptr);

            format!("xload8_u32_offset32 {dst}, {ptr}, {offset}")
        }
        
        RawInst::XLoad8S32Offset32 { dst,ptr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let ptr = reg_name(**ptr);

            format!("xload8_s32_offset32 {dst}, {ptr}, {offset}")
        }
        
        RawInst::XLoad16LeU32Offset32 { dst,ptr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let ptr = reg_name(**ptr);

            format!("xload16le_u32_offset32 {dst}, {ptr}, {offset}")
        }
        
        RawInst::XLoad16LeS32Offset32 { dst,ptr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let ptr = reg_name(**ptr);

            format!("xload16le_s32_offset32 {dst}, {ptr}, {offset}")
        }
        
        RawInst::XLoad32LeOffset32 { dst,ptr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let ptr = reg_name(**ptr);

            format!("xload32le_offset32 {dst}, {ptr}, {offset}")
        }
        
        RawInst::XLoad8U64Offset32 { dst,ptr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let ptr = reg_name(**ptr);

            format!("xload8_u64_offset32 {dst}, {ptr}, {offset}")
        }
        
        RawInst::XLoad8S64Offset32 { dst,ptr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let ptr = reg_name(**ptr);

            format!("xload8_s64_offset32 {dst}, {ptr}, {offset}")
        }
        
        RawInst::XLoad16LeU64Offset32 { dst,ptr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let ptr = reg_name(**ptr);

            format!("xload16le_u64_offset32 {dst}, {ptr}, {offset}")
        }
        
        RawInst::XLoad16LeS64Offset32 { dst,ptr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let ptr = reg_name(**ptr);

            format!("xload16le_s64_offset32 {dst}, {ptr}, {offset}")
        }
        
        RawInst::XLoad32LeU64Offset32 { dst,ptr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let ptr = reg_name(**ptr);

            format!("xload32le_u64_offset32 {dst}, {ptr}, {offset}")
        }
        
        RawInst::XLoad32LeS64Offset32 { dst,ptr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let ptr = reg_name(**ptr);

            format!("xload32le_s64_offset32 {dst}, {ptr}, {offset}")
        }
        
        RawInst::XLoad64LeOffset32 { dst,ptr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let ptr = reg_name(**ptr);

            format!("xload64le_offset32 {dst}, {ptr}, {offset}")
        }
        
        RawInst::XStore8Offset32 { ptr,offset,src, } => {
            let ptr = reg_name(**ptr);
let src = reg_name(**src);

            format!("xstore8_offset32 {ptr}, {offset}, {src}")
        }
        
        RawInst::XStore16LeOffset32 { ptr,offset,src, } => {
            let ptr = reg_name(**ptr);
let src = reg_name(**src);

            format!("xstore16le_offset32 {ptr}, {offset}, {src}")
        }
        
        RawInst::XStore32LeOffset32 { ptr,offset,src, } => {
            let ptr = reg_name(**ptr);
let src = reg_name(**src);

            format!("xstore32le_offset32 {ptr}, {offset}, {src}")
        }
        
        RawInst::XStore64LeOffset32 { ptr,offset,src, } => {
            let ptr = reg_name(**ptr);
let src = reg_name(**src);

            format!("xstore64le_offset32 {ptr}, {offset}, {src}")
        }
        
        RawInst::XLoad8U32Offset8 { dst,ptr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let ptr = reg_name(**ptr);

            format!("xload8_u32_offset8 {dst}, {ptr}, {offset}")
        }
        
        RawInst::XLoad8S32Offset8 { dst,ptr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let ptr = reg_name(**ptr);

            format!("xload8_s32_offset8 {dst}, {ptr}, {offset}")
        }
        
        RawInst::XLoad16LeU32Offset8 { dst,ptr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let ptr = reg_name(**ptr);

            format!("xload16le_u32_offset8 {dst}, {ptr}, {offset}")
        }
        
        RawInst::XLoad16LeS32Offset8 { dst,ptr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let ptr = reg_name(**ptr);

            format!("xload16le_s32_offset8 {dst}, {ptr}, {offset}")
        }
        
        RawInst::XLoad32LeOffset8 { dst,ptr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let ptr = reg_name(**ptr);

            format!("xload32le_offset8 {dst}, {ptr}, {offset}")
        }
        
        RawInst::XLoad8U64Offset8 { dst,ptr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let ptr = reg_name(**ptr);

            format!("xload8_u64_offset8 {dst}, {ptr}, {offset}")
        }
        
        RawInst::XLoad8S64Offset8 { dst,ptr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let ptr = reg_name(**ptr);

            format!("xload8_s64_offset8 {dst}, {ptr}, {offset}")
        }
        
        RawInst::XLoad16LeU64Offset8 { dst,ptr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let ptr = reg_name(**ptr);

            format!("xload16le_u64_offset8 {dst}, {ptr}, {offset}")
        }
        
        RawInst::XLoad16LeS64Offset8 { dst,ptr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let ptr = reg_name(**ptr);

            format!("xload16le_s64_offset8 {dst}, {ptr}, {offset}")
        }
        
        RawInst::XLoad32LeU64Offset8 { dst,ptr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let ptr = reg_name(**ptr);

            format!("xload32le_u64_offset8 {dst}, {ptr}, {offset}")
        }
        
        RawInst::XLoad32LeS64Offset8 { dst,ptr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let ptr = reg_name(**ptr);

            format!("xload32le_s64_offset8 {dst}, {ptr}, {offset}")
        }
        
        RawInst::XLoad64LeOffset8 { dst,ptr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let ptr = reg_name(**ptr);

            format!("xload64le_offset8 {dst}, {ptr}, {offset}")
        }
        
        RawInst::XStore8Offset8 { ptr,offset,src, } => {
            let ptr = reg_name(**ptr);
let src = reg_name(**src);

            format!("xstore8_offset8 {ptr}, {offset}, {src}")
        }
        
        RawInst::XStore16LeOffset8 { ptr,offset,src, } => {
            let ptr = reg_name(**ptr);
let src = reg_name(**src);

            format!("xstore16le_offset8 {ptr}, {offset}, {src}")
        }
        
        RawInst::XStore32LeOffset8 { ptr,offset,src, } => {
            let ptr = reg_name(**ptr);
let src = reg_name(**src);

            format!("xstore32le_offset8 {ptr}, {offset}, {src}")
        }
        
        RawInst::XStore64LeOffset8 { ptr,offset,src, } => {
            let ptr = reg_name(**ptr);
let src = reg_name(**src);

            format!("xstore64le_offset8 {ptr}, {offset}, {src}")
        }
        
        RawInst::XLoad8U32G32 { dst,base,addr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let base = reg_name(**base);
let addr = reg_name(**addr);

            format!("xload8_u32_g32 {dst}, {base}, {addr}, {offset}")
        }
        
        RawInst::XLoad8S32G32 { dst,base,addr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let base = reg_name(**base);
let addr = reg_name(**addr);

            format!("xload8_s32_g32 {dst}, {base}, {addr}, {offset}")
        }
        
        RawInst::XLoad16LeU32G32 { dst,base,addr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let base = reg_name(**base);
let addr = reg_name(**addr);

            format!("xload16le_u32_g32 {dst}, {base}, {addr}, {offset}")
        }
        
        RawInst::XLoad16LeS32G32 { dst,base,addr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let base = reg_name(**base);
let addr = reg_name(**addr);

            format!("xload16le_s32_g32 {dst}, {base}, {addr}, {offset}")
        }
        
        RawInst::XLoad32LeG32 { dst,base,addr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let base = reg_name(**base);
let addr = reg_name(**addr);

            format!("xload32le_g32 {dst}, {base}, {addr}, {offset}")
        }
        
        RawInst::XLoad64LeG32 { dst,base,addr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let base = reg_name(**base);
let addr = reg_name(**addr);

            format!("xload64le_g32 {dst}, {base}, {addr}, {offset}")
        }
        
        RawInst::XStore8G32 { base,addr,offset,src, } => {
            let base = reg_name(**base);
let addr = reg_name(**addr);
let src = reg_name(**src);

            format!("xstore8_g32 {base}, {addr}, {offset}, {src}")
        }
        
        RawInst::XStore16LeG32 { base,addr,offset,src, } => {
            let base = reg_name(**base);
let addr = reg_name(**addr);
let src = reg_name(**src);

            format!("xstore16le_g32 {base}, {addr}, {offset}, {src}")
        }
        
        RawInst::XStore32LeG32 { base,addr,offset,src, } => {
            let base = reg_name(**base);
let addr = reg_name(**addr);
let src = reg_name(**src);

            format!("xstore32le_g32 {base}, {addr}, {offset}, {src}")
        }
        
        RawInst::XStore64LeG32 { base,addr,offset,src, } => {
            let base = reg_name(**base);
let addr = reg_name(**addr);
let src = reg_name(**src);

            format!("xstore64le_g32 {base}, {addr}, {offset}, {src}")
        }
...
let if_zero = reg_name(**if_zero);
        ...
        RawInst::XBc32BoundTrap { addr,bound,size,code, } => {
            let addr = reg_name(**addr);
let bound = reg_name(**bound);

            format!("xbc32_bound_trap {addr}, {bound}, {size} // trap={code:?}")
        }
        
        RawInst::XBc32BoundNeTrap { addr,bound_ptr,bound_off,size,code, } => {
            let addr = reg_name(**addr);
let bound_ptr = reg_name(**bound_ptr);

            format!("xbc32_boundne_trap {addr}, {bound_ptr}, {bound_off}, {size} // trap={code:?}")
        }
        
        RawInst::XBc32StrictBoundTrap { addr,bound,code, } => {
            let addr = reg_name(**addr);
let bound = reg_name(**bound);

            format!("xbc32_strict_bound_trap {addr}, {bound} // trap={code:?}")
        }
        
        RawInst::XBc32StrictBoundNeTrap { addr,bound_ptr,bound_off,code, } => {
            let addr = reg_name(**addr);
let bound_ptr = reg_name(**bound_ptr);

            format!("xbc32_strict_boundne_trap {addr}, {bound_ptr}, {bound_off} // trap={code:?}")
        }
...
let src = reg_name(**src);
        ...
        RawInst::XLoad16BeU64Offset32 { dst,ptr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let ptr = reg_name(**ptr);

            format!("xload16be_u64_offset32 {dst}, {ptr}, {offset}")
        }
        
        RawInst::XLoad16BeS64Offset32 { dst,ptr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let ptr = reg_name(**ptr);

            format!("xload16be_s64_offset32 {dst}, {ptr}, {offset}")
        }
        
        RawInst::XLoad32BeU64Offset32 { dst,ptr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let ptr = reg_name(**ptr);

            format!("xload32be_u64_offset32 {dst}, {ptr}, {offset}")
        }
        
        RawInst::XLoad32BeS64Offset32 { dst,ptr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let ptr = reg_name(**ptr);

            format!("xload32be_s64_offset32 {dst}, {ptr}, {offset}")
        }
        
        RawInst::XLoad64BeOffset32 { dst,ptr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let ptr = reg_name(**ptr);

            format!("xload64be_offset32 {dst}, {ptr}, {offset}")
        }
        
        RawInst::XStore16BeOffset32 { ptr,offset,src, } => {
            let ptr = reg_name(**ptr);
let src = reg_name(**src);

            format!("xstore16be_offset32 {ptr}, {offset}, {src}")
        }
        
        RawInst::XStore32BeOffset32 { ptr,offset,src, } => {
            let ptr = reg_name(**ptr);
let src = reg_name(**src);

            format!("xstore32be_offset32 {ptr}, {offset}, {src}")
        }
        
        RawInst::XStore64BeOffset32 { ptr,offset,src, } => {
            let ptr = reg_name(**ptr);
let src = reg_name(**src);

            format!("xstore64be_offset32 {ptr}, {offset}, {src}")
        }
        
        RawInst::Fload32BeOffset32 { dst,ptr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let ptr = reg_name(**ptr);

            format!("fload32be_offset32 {dst}, {ptr}, {offset}")
        }
        
        RawInst::Fload64BeOffset32 { dst,ptr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let ptr = reg_name(**ptr);

            format!("fload64be_offset32 {dst}, {ptr}, {offset}")
        }
        
        RawInst::Fstore32BeOffset32 { ptr,offset,src, } => {
            let ptr = reg_name(**ptr);
let src = reg_name(**src);

            format!("fstore32be_offset32 {ptr}, {offset}, {src}")
        }
        
        RawInst::Fstore64BeOffset32 { ptr,offset,src, } => {
            let ptr = reg_name(**ptr);
let src = reg_name(**src);

            format!("fstore64be_offset32 {ptr}, {offset}, {src}")
        }
        
        RawInst::Fload32LeOffset32 { dst,ptr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let ptr = reg_name(**ptr);

            format!("fload32le_offset32 {dst}, {ptr}, {offset}")
        }
        
        RawInst::Fload64LeOffset32 { dst,ptr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let ptr = reg_name(**ptr);

            format!("fload64le_offset32 {dst}, {ptr}, {offset}")
        }
        
        RawInst::Fstore32LeOffset32 { ptr,offset,src, } => {
            let ptr = reg_name(**ptr);
let src = reg_name(**src);

            format!("fstore32le_offset32 {ptr}, {offset}, {src}")
        }
        
        RawInst::Fstore64LeOffset32 { ptr,offset,src, } => {
            let ptr = reg_name(**ptr);
let src = reg_name(**src);

            format!("fstore64le_offset32 {ptr}, {offset}, {src}")
        }
        
        RawInst::VLoad128Offset32 { dst,ptr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let ptr = reg_name(**ptr);

            format!("vload128le_offset32 {dst}, {ptr}, {offset}")
        }
        
        RawInst::Vstore128LeOffset32 { ptr,offset,src, } => {
            let ptr = reg_name(**ptr);
let src = reg_name(**src);

            format!("vstore128le_offset32 {ptr}, {offset}, {src}")
        }
...
let src = reg_name(**src);
        ...
        RawInst::VLoad8x8SOffset32 { dst,ptr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let ptr = reg_name(**ptr);

            format!("vload8x8_s_offset32 {dst}, {ptr}, {offset}")
        }
        
        RawInst::VLoad8x8UOffset32 { dst,ptr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let ptr = reg_name(**ptr);

            format!("vload8x8_u_offset32 {dst}, {ptr}, {offset}")
        }
        
        RawInst::VLoad16x4LeSOffset32 { dst,ptr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let ptr = reg_name(**ptr);

            format!("vload16x4le_s_offset32 {dst}, {ptr}, {offset}")
        }
        
        RawInst::VLoad16x4LeUOffset32 { dst,ptr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let ptr = reg_name(**ptr);

            format!("vload16x4le_u_offset32 {dst}, {ptr}, {offset}")
        }
        
        RawInst::VLoad32x2LeSOffset32 { dst,ptr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let ptr = reg_name(**ptr);

            format!("vload32x2le_s_offset32 {dst}, {ptr}, {offset}")
        }
        
        RawInst::VLoad32x2LeUOffset32 { dst,ptr,offset, } => {
            let dst = reg_name(*dst.to_reg());
let ptr = reg_name(**ptr);

            format!("vload32x2le_u_offset32 {dst}, {ptr}, {offset}")
        }
...
collector.reg_use(src2);
        ...
        RawInst::XLoad8U32Offset32 { dst,ptr, .. } => {
            collector.reg_use(ptr);

            collector.reg_def(dst);

        }
        
        RawInst::XLoad8S32Offset32 { dst,ptr, .. } => {
            collector.reg_use(ptr);
        ...
        RawInst::XLoad16LeU32Offset32 { dst,ptr, .. } => {
            collector.reg_use(ptr);
        ...
        RawInst::XLoad16LeS32Offset32 { dst,ptr, .. } => {
            collector.reg_use(ptr);
        ...
        RawInst::XLoad32LeOffset32 { dst,ptr, .. } => {
            collector.reg_use(ptr);
        ...
        RawInst::XLoad8U64Offset32 { dst,ptr, .. } => {
            collector.reg_use(ptr);
        ...
        RawInst::XLoad8S64Offset32 { dst,ptr, .. } => {
            collector.reg_use(ptr);
        ...
        RawInst::XLoad16LeU64Offset32 { dst,ptr, .. } => {
            collector.reg_use(ptr);
        ...
        RawInst::XLoad16LeS64Offset32 { dst,ptr, .. } => {
            collector.reg_use(ptr);
        ...
        RawInst::XLoad32LeU64Offset32 { dst,ptr, .. } => {
            collector.reg_use(ptr);
        ...
        RawInst::XLoad32LeS64Offset32 { dst,ptr, .. } => {
            collector.reg_use(ptr);
        ...
        RawInst::XLoad64LeOffset32 { dst,ptr, .. } => {
            collector.reg_use(ptr);
        ...
        RawInst::XStore8Offset32 { ptr,src, .. } => {
            collector.reg_use(ptr);
collector.reg_use(src);
        ...
        RawInst::XStore16LeOffset32 { ptr,src, .. } => {
            collector.reg_use(ptr);
collector.reg_use(src);
        ...
        RawInst::XStore32LeOffset32 { ptr,src, .. } => {
            collector.reg_use(ptr);
collector.reg_use(src);
        ...
        RawInst::XStore64LeOffset32 { ptr,src, .. } => {
            collector.reg_use(ptr);
collector.reg_use(src);
        ...
        RawInst::XLoad8U32Offset8 { dst,ptr, .. } => {
            collector.reg_use(ptr);
        ...
        RawInst::XLoad8S32Offset8 { dst,ptr, .. } => {
            collector.reg_use(ptr);
        ...
        RawInst::XLoad16LeU32Offset8 { dst,ptr, .. } => {
            collector.reg_use(ptr);
        ...
        RawInst::XLoad16LeS32Offset8 { dst,ptr, .. } => {
            collector.reg_use(ptr);
        ...
        RawInst::XLoad32LeOffset8 { dst,ptr, .. } => {
            collector.reg_use(ptr);
        ...
        RawInst::XLoad8U64Offset8 { dst,ptr, .. } => {
            collector.reg_use(ptr);
        ...
        RawInst::XLoad8S64Offset8 { dst,ptr, .. } => {
            collector.reg_use(ptr);
        ...
        RawInst::XLoad16LeU64Offset8 { dst,ptr, .. } => {
            collector.reg_use(ptr);
        ...
        RawInst::XLoad16LeS64Offset8 { dst,ptr, .. } => {
            collector.reg_use(ptr);
        ...
        RawInst::XLoad32LeU64Offset8 { dst,ptr, .. } => {
            collector.reg_use(ptr);
        ...
        RawInst::XLoad32LeS64Offset8 { dst,ptr, .. } => {
            collector.reg_use(ptr);
        ...
        RawInst::XLoad64LeOffset8 { dst,ptr, .. } => {
            collector.reg_use(ptr);
        ...
        RawInst::XStore8Offset8 { ptr,src, .. } => {
            collector.reg_use(ptr);
collector.reg_use(src);
        ...
        RawInst::XStore16LeOffset8 { ptr,src, .. } => {
            collector.reg_use(ptr);
collector.reg_use(src);
        ...
        RawInst::XStore32LeOffset8 { ptr,src, .. } => {
            collector.reg_use(ptr);
collector.reg_use(src);
        ...
        RawInst::XStore64LeOffset8 { ptr,src, .. } => {
            collector.reg_use(ptr);
collector.reg_use(src);
        ...
        RawInst::XLoad8U32G32 { dst,base,addr, .. } => {
...
collector.reg_use(addr);
        ...
        RawInst::XLoad8S32G32 { dst,base,addr, .. } => {
            collector.reg_use(base);
collector.reg_use(addr);
        ...
        RawInst::XLoad16LeU32G32 { dst,base,addr, .. } => {
            collector.reg_use(base);
collector.reg_use(addr);
        ...
        RawInst::XLoad16LeS32G32 { dst,base,addr, .. } => {
            collector.reg_use(base);
collector.reg_use(addr);
        ...
        RawInst::XLoad32LeG32 { dst,base,addr, .. } => {
            collector.reg_use(base);
collector.reg_use(addr);
        ...
        RawInst::XLoad64LeG32 { dst,base,addr, .. } => {
            collector.reg_use(base);
collector.reg_use(addr);
        ...
        RawInst::XStore8G32 { base,addr,src, .. } => {
            collector.reg_use(base);
collector.reg_use(addr);
collector.reg_use(src);
        ...
        RawInst::XStore16LeG32 { base,addr,src, .. } => {
            collector.reg_use(base);
collector.reg_use(addr);
collector.reg_use(src);
        ...
        RawInst::XStore32LeG32 { base,addr,src, .. } => {
            collector.reg_use(base);
collector.reg_use(addr);
collector.reg_use(src);
        ...
        RawInst::XStore64LeG32 { base,addr,src, .. } => {
            collector.reg_use(base);
collector.reg_use(addr);
collector.reg_use(src);
...
collector.reg_use(if_zero);
        ...
        RawInst::XBc32BoundTrap { addr,bound, .. } => {
            collector.reg_use(addr);
collector.reg_use(bound);

            
        }
        
        RawInst::XBc32BoundNeTrap { addr,bound_ptr, .. } => {
            collector.reg_use(addr);
collector.reg_use(bound_ptr);

            
        }
        
        RawInst::XBc32StrictBoundTrap { addr,bound, .. } => {
            collector.reg_use(addr);
collector.reg_use(bound);
        ...
        RawInst::XBc32StrictBoundNeTrap { addr,bound_ptr, .. } => {
            collector.reg_use(addr);
collector.reg_use(bound_ptr);
...
collector.reg_use(src2);
        ...
        RawInst::XLoad16BeU64Offset32 { dst,ptr, .. } => {
            collector.reg_use(ptr);
        ...
        RawInst::XLoad16BeS64Offset32 { dst,ptr, .. } => {
            collector.reg_use(ptr);
        ...
        RawInst::XLoad32BeU64Offset32 { dst,ptr, .. } => {
            collector.reg_use(ptr);
        ...
        RawInst::XLoad32BeS64Offset32 { dst,ptr, .. } => {
            collector.reg_use(ptr);
        ...
        RawInst::XLoad64BeOffset32 { dst,ptr, .. } => {
            collector.reg_use(ptr);
        ...
        RawInst::XStore16BeOffset32 { ptr,src, .. } => {
            collector.reg_use(ptr);
collector.reg_use(src);
        ...
        RawInst::XStore32BeOffset32 { ptr,src, .. } => {
            collector.reg_use(ptr);
collector.reg_use(src);
        ...
        RawInst::XStore64BeOffset32 { ptr,src, .. } => {
            collector.reg_use(ptr);
collector.reg_use(src);
        ...
        RawInst::Fload32BeOffset32 { dst,ptr, .. } => {
            collector.reg_use(ptr);
        ...
        RawInst::Fload64BeOffset32 { dst,ptr, .. } => {
            collector.reg_use(ptr);
        ...
        RawInst::Fstore32BeOffset32 { ptr,src, .. } => {
            collector.reg_use(ptr);
collector.reg_use(src);
        ...
        RawInst::Fstore64BeOffset32 { ptr,src, .. } => {
            collector.reg_use(ptr);
collector.reg_use(src);
        ...
        RawInst::Fload32LeOffset32 { dst,ptr, .. } => {
            collector.reg_use(ptr);
        ...
        RawInst::Fload64LeOffset32 { dst,ptr, .. } => {
            collector.reg_use(ptr);
        ...
        RawInst::Fstore32LeOffset32 { ptr,src, .. } => {
            collector.reg_use(ptr);
collector.reg_use(src);
        ...
        RawInst::Fstore64LeOffset32 { ptr,src, .. } => {
            collector.reg_use(ptr);
collector.reg_use(src);
        ...
        RawInst::VLoad128Offset32 { dst,ptr, .. } => {
            collector.reg_use(ptr);
        ...
        RawInst::Vstore128LeOffset32 { ptr,src, .. } => {
            collector.reg_use(ptr);
collector.reg_use(src);
...
collector.reg_use(src2);
        ...
        RawInst::VLoad8x8SOffset32 { dst,ptr, .. } => {
            collector.reg_use(ptr);
        ...
        RawInst::VLoad8x8UOffset32 { dst,ptr, .. } => {
            collector.reg_use(ptr);
        ...
        RawInst::VLoad16x4LeSOffset32 { dst,ptr, .. } => {
            collector.reg_use(ptr);
        ...
        RawInst::VLoad16x4LeUOffset32 { dst,ptr, .. } => {
            collector.reg_use(ptr);
        ...
        RawInst::VLoad32x2LeSOffset32 { dst,ptr, .. } => {
            collector.reg_use(ptr);
        ...
        RawInst::VLoad32x2LeUOffset32 { dst,ptr, .. } => {
            collector.reg_use(ptr);
...
match *inst {
        ...
        RawInst::XLoad8U32Offset32 { dst,ptr,offset, } => {
            ...
            pulley_interpreter::encode::xload8_u32_offset32(sink, dst,ptr,offset,)
        }
        
        RawInst::XLoad8S32Offset32 { dst,ptr,offset, } => {
            
            pulley_interpreter::encode::xload8_s32_offset32(sink, dst,ptr,offset,)
        }
        
        RawInst::XLoad16LeU32Offset32 { dst,ptr,offset, } => {
            
            pulley_interpreter::encode::xload16le_u32_offset32(sink, dst,ptr,offset,)
        }
        
        RawInst::XLoad16LeS32Offset32 { dst,ptr,offset, } => {
            
            pulley_interpreter::encode::xload16le_s32_offset32(sink, dst,ptr,offset,)
        }
        
        RawInst::XLoad32LeOffset32 { dst,ptr,offset, } => {
            
            pulley_interpreter::encode::xload32le_offset32(sink, dst,ptr,offset,)
        }
        
        RawInst::XLoad8U64Offset32 { dst,ptr,offset, } => {
            
            pulley_interpreter::encode::xload8_u64_offset32(sink, dst,ptr,offset,)
        }
        
        RawInst::XLoad8S64Offset32 { dst,ptr,offset, } => {
            
            pulley_interpreter::encode::xload8_s64_offset32(sink, dst,ptr,offset,)
        }
        
        RawInst::XLoad16LeU64Offset32 { dst,ptr,offset, } => {
            
            pulley_interpreter::encode::xload16le_u64_offset32(sink, dst,ptr,offset,)
        }
        
        RawInst::XLoad16LeS64Offset32 { dst,ptr,offset, } => {
            
            pulley_interpreter::encode::xload16le_s64_offset32(sink, dst,ptr,offset,)
        }
        
        RawInst::XLoad32LeU64Offset32 { dst,ptr,offset, } => {
            
            pulley_interpreter::encode::xload32le_u64_offset32(sink, dst,ptr,offset,)
        }
        
        RawInst::XLoad32LeS64Offset32 { dst,ptr,offset, } => {
            
            pulley_interpreter::encode::xload32le_s64_offset32(sink, dst,ptr,offset,)
        }
        
        RawInst::XLoad64LeOffset32 { dst,ptr,offset, } => {
            
            pulley_interpreter::encode::xload64le_offset32(sink, dst,ptr,offset,)
        }
        
        RawInst::XStore8Offset32 { ptr,offset,src, } => {
            
            pulley_interpreter::encode::xstore8_offset32(sink, ptr,offset,src,)
        }
        
        RawInst::XStore16LeOffset32 { ptr,offset,src, } => {
            
            pulley_interpreter::encode::xstore16le_offset32(sink, ptr,offset,src,)
        }
        
        RawInst::XStore32LeOffset32 { ptr,offset,src, } => {
            
            pulley_interpreter::encode::xstore32le_offset32(sink, ptr,offset,src,)
        }
        
        RawInst::XStore64LeOffset32 { ptr,offset,src, } => {
            
            pulley_interpreter::encode::xstore64le_offset32(sink, ptr,offset,src,)
        }
        
        RawInst::XLoad8U32Offset8 { dst,ptr,offset, } => {
            
            pulley_interpreter::encode::xload8_u32_offset8(sink, dst,ptr,offset,)
        }
        
        RawInst::XLoad8S32Offset8 { dst,ptr,offset, } => {
            
            pulley_interpreter::encode::xload8_s32_offset8(sink, dst,ptr,offset,)
        }
        
        RawInst::XLoad16LeU32Offset8 { dst,ptr,offset, } => {
            
            pulley_interpreter::encode::xload16le_u32_offset8(sink, dst,ptr,offset,)
        }
        
        RawInst::XLoad16LeS32Offset8 { dst,ptr,offset, } => {
            
            pulley_interpreter::encode::xload16le_s32_offset8(sink, dst,ptr,offset,)
        }
        
        RawInst::XLoad32LeOffset8 { dst,ptr,offset, } => {
            
            pulley_interpreter::encode::xload32le_offset8(sink, dst,ptr,offset,)
        }
        
        RawInst::XLoad8U64Offset8 { dst,ptr,offset, } => {
            
            pulley_interpreter::encode::xload8_u64_offset8(sink, dst,ptr,offset,)
        }
        
        RawInst::XLoad8S64Offset8 { dst,ptr,offset, } => {
            
            pulley_interpreter::encode::xload8_s64_offset8(sink, dst,ptr,offset,)
        }
        
        RawInst::XLoad16LeU64Offset8 { dst,ptr,offset, } => {
            
            pulley_interpreter::encode::xload16le_u64_offset8(sink, dst,ptr,offset,)
        }
        
        RawInst::XLoad16LeS64Offset8 { dst,ptr,offset, } => {
            
            pulley_interpreter::encode::xload16le_s64_offset8(sink, dst,ptr,offset,)
        }
        
        RawInst::XLoad32LeU64Offset8 { dst,ptr,offset, } => {
            
            pulley_interpreter::encode::xload32le_u64_offset8(sink, dst,ptr,offset,)
        }
        
        RawInst::XLoad32LeS64Offset8 { dst,ptr,offset, } => {
            
            pulley_interpreter::encode::xload32le_s64_offset8(sink, dst,ptr,offset,)
        }
        
        RawInst::XLoad64LeOffset8 { dst,ptr,offset, } => {
            
            pulley_interpreter::encode::xload64le_offset8(sink, dst,ptr,offset,)
        }
        
        RawInst::XStore8Offset8 { ptr,offset,src, } => {
            
            pulley_interpreter::encode::xstore8_offset8(sink, ptr,offset,src,)
        }
        
        RawInst::XStore16LeOffset8 { ptr,offset,src, } => {
            
            pulley_interpreter::encode::xstore16le_offset8(sink, ptr,offset,src,)
        }
        
        RawInst::XStore32LeOffset8 { ptr,offset,src, } => {
            
            pulley_interpreter::encode::xstore32le_offset8(sink, ptr,offset,src,)
        }
        
        RawInst::XStore64LeOffset8 { ptr,offset,src, } => {
            
            pulley_interpreter::encode::xstore64le_offset8(sink, ptr,offset,src,)
        }
        
        RawInst::XLoad8U32G32 { dst,base,addr,offset, } => {
            
            pulley_interpreter::encode::xload8_u32_g32(sink, dst,base,addr,offset,)
        }
        
        RawInst::XLoad8S32G32 { dst,base,addr,offset, } => {
            
            pulley_interpreter::encode::xload8_s32_g32(sink, dst,base,addr,offset,)
        }
        
        RawInst::XLoad16LeU32G32 { dst,base,addr,offset, } => {
            
            pulley_interpreter::encode::xload16le_u32_g32(sink, dst,base,addr,offset,)
        }
        
        RawInst::XLoad16LeS32G32 { dst,base,addr,offset, } => {
            
            pulley_interpreter::encode::xload16le_s32_g32(sink, dst,base,addr,offset,)
        }
        
        RawInst::XLoad32LeG32 { dst,base,addr,offset, } => {
            
            pulley_interpreter::encode::xload32le_g32(sink, dst,base,addr,offset,)
        }
        
        RawInst::XLoad64LeG32 { dst,base,addr,offset, } => {
            
            pulley_interpreter::encode::xload64le_g32(sink, dst,base,addr,offset,)
        }
        
        RawInst::XStore8G32 { base,addr,offset,src, } => {
            
            pulley_interpreter::encode::xstore8_g32(sink, base,addr,offset,src,)
        }
        
        RawInst::XStore16LeG32 { base,addr,offset,src, } => {
            
            pulley_interpreter::encode::xstore16le_g32(sink, base,addr,offset,src,)
        }
        
        RawInst::XStore32LeG32 { base,addr,offset,src, } => {
            
            pulley_interpreter::encode::xstore32le_g32(sink, base,addr,offset,src,)
        }
        
        RawInst::XStore64LeG32 { base,addr,offset,src, } => {
            
            pulley_interpreter::encode::xstore64le_g32(sink, base,addr,offset,src,)
        }
        ...
        RawInst::XBc32BoundTrap { addr,bound,size,code, } => {
            ...
            pulley_interpreter::encode::xbc32_bound_trap(sink, addr,bound,size,)
        }
        
        RawInst::XBc32BoundNeTrap { addr,bound_ptr,bound_off,size,code, } => {
            sink.add_trap(code);

            pulley_interpreter::encode::xbc32_boundne_trap(sink, addr,bound_ptr,bound_off,size,)
        }
        
        RawInst::XBc32StrictBoundTrap { addr,bound,code, } => {
            sink.add_trap(code);

            pulley_interpreter::encode::xbc32_strict_bound_trap(sink, addr,bound,)
        }
        
        RawInst::XBc32StrictBoundNeTrap { addr,bound_ptr,bound_off,code, } => {
            sink.add_trap(code);

            pulley_interpreter::encode::xbc32_strict_boundne_trap(sink, addr,bound_ptr,bound_off,)
        }
        ...
        RawInst::XLoad16BeU64Offset32 { dst,ptr,offset, } => {
            ...
            pulley_interpreter::encode::xload16be_u64_offset32(sink, dst,ptr,offset,)
        }
        
        RawInst::XLoad16BeS64Offset32 { dst,ptr,offset, } => {
            
            pulley_interpreter::encode::xload16be_s64_offset32(sink, dst,ptr,offset,)
        }
        
        RawInst::XLoad32BeU64Offset32 { dst,ptr,offset, } => {
            
            pulley_interpreter::encode::xload32be_u64_offset32(sink, dst,ptr,offset,)
        }
        
        RawInst::XLoad32BeS64Offset32 { dst,ptr,offset, } => {
            
            pulley_interpreter::encode::xload32be_s64_offset32(sink, dst,ptr,offset,)
        }
        
        RawInst::XLoad64BeOffset32 { dst,ptr,offset, } => {
            
            pulley_interpreter::encode::xload64be_offset32(sink, dst,ptr,offset,)
        }
        
        RawInst::XStore16BeOffset32 { ptr,offset,src, } => {
            
            pulley_interpreter::encode::xstore16be_offset32(sink, ptr,offset,src,)
        }
        
        RawInst::XStore32BeOffset32 { ptr,offset,src, } => {
            
            pulley_interpreter::encode::xstore32be_offset32(sink, ptr,offset,src,)
        }
        
        RawInst::XStore64BeOffset32 { ptr,offset,src, } => {
            
            pulley_interpreter::encode::xstore64be_offset32(sink, ptr,offset,src,)
        }
        
        RawInst::Fload32BeOffset32 { dst,ptr,offset, } => {
            
            pulley_interpreter::encode::fload32be_offset32(sink, dst,ptr,offset,)
        }
        
        RawInst::Fload64BeOffset32 { dst,ptr,offset, } => {
            
            pulley_interpreter::encode::fload64be_offset32(sink, dst,ptr,offset,)
        }
        
        RawInst::Fstore32BeOffset32 { ptr,offset,src, } => {
            
            pulley_interpreter::encode::fstore32be_offset32(sink, ptr,offset,src,)
        }
        
        RawInst::Fstore64BeOffset32 { ptr,offset,src, } => {
            
            pulley_interpreter::encode::fstore64be_offset32(sink, ptr,offset,src,)
        }
        
        RawInst::Fload32LeOffset32 { dst,ptr,offset, } => {
            
            pulley_interpreter::encode::fload32le_offset32(sink, dst,ptr,offset,)
        }
        
        RawInst::Fload64LeOffset32 { dst,ptr,offset, } => {
            
            pulley_interpreter::encode::fload64le_offset32(sink, dst,ptr,offset,)
        }
        
        RawInst::Fstore32LeOffset32 { ptr,offset,src, } => {
            
            pulley_interpreter::encode::fstore32le_offset32(sink, ptr,offset,src,)
        }
        
        RawInst::Fstore64LeOffset32 { ptr,offset,src, } => {
            
            pulley_interpreter::encode::fstore64le_offset32(sink, ptr,offset,src,)
        }
        
        RawInst::VLoad128Offset32 { dst,ptr,offset, } => {
            
            pulley_interpreter::encode::vload128le_offset32(sink, dst,ptr,offset,)
        }
        
        RawInst::Vstore128LeOffset32 { ptr,offset,src, } => {
            
            pulley_interpreter::encode::vstore128le_offset32(sink, ptr,offset,src,)
        }
        ...
        RawInst::VLoad8x8SOffset32 { dst,ptr,offset, } => {
            ...
            pulley_interpreter::encode::vload8x8_s_offset32(sink, dst,ptr,offset,)
        }
        
        RawInst::VLoad8x8UOffset32 { dst,ptr,offset, } => {
            
            pulley_interpreter::encode::vload8x8_u_offset32(sink, dst,ptr,offset,)
        }
        
        RawInst::VLoad16x4LeSOffset32 { dst,ptr,offset, } => {
            
            pulley_interpreter::encode::vload16x4le_s_offset32(sink, dst,ptr,offset,)
        }
        
        RawInst::VLoad16x4LeUOffset32 { dst,ptr,offset, } => {
            
            pulley_interpreter::encode::vload16x4le_u_offset32(sink, dst,ptr,offset,)
        }
        
        RawInst::VLoad32x2LeSOffset32 { dst,ptr,offset, } => {
            
            pulley_interpreter::encode::vload32x2le_s_offset32(sink, dst,ptr,offset,)
        }
        
        RawInst::VLoad32x2LeUOffset32 { dst,ptr,offset, } => {
            
            pulley_interpreter::encode::vload32x2le_u_offset32(sink, dst,ptr,offset,)
        }
